...
const STEP_TO_EVENT_TYPE = {
  ...
  "记录": EVENT_TYPES.NOTE_ADDED,
  "观察": EVENT_TYPES.NOTE_ADDED,
...
function extractRuleInfoFromLog(logNote) {
  ...
  if (logNote.includes("可抄纸") || logNote.includes("达到标准")) {
    result.hasEvaluation = true;
    result.nextStatus = "可抄纸";
    if (logNote.includes("天数达标")) result.reasons.push("天数达标");
    if (logNote.includes("温度正常")) result.reasons.push("温度正常");
    if (logNote.includes("无异常")) result.reasons.push("无异常关键词");
  }

  if (logNote.includes("发酵中") && !logNote.includes("可抄纸")) {
    result.hasEvaluation = true;
    result.nextStatus = "发酵中";
  }

  if (logNote.includes("异常") || logNote.includes("霉") || logNote.includes("臭") || logNote.includes("结块")) {
    result.hasEvaluation = true;
    result.nextStatus = "异常观察";
    if (logNote.includes("霉")) result.reasons.push("关键词异常：霉");
    if (logNote.includes("臭")) result.reasons.push("关键词异常：臭");
    if (logNote.includes("结块")) result.reasons.push("关键词异常：结块");
  }

  if (logNote.includes("温度") && (logNote.includes("低于") || logNote.includes("高于") || logNote.includes("超限"))) {
    result.hasEvaluation = true;
    result.reasons.push("温度超限");
  }

  if (logNote.includes("判定依据") || logNote.includes("规则")) {
    result.hasEvaluation = true;
    const match = logNote.match(/判定依据[：:]\s*([^（]+)/);
    if (match) {
      result.reasons = match[1].split(/[；;，,]/).map(s => s.trim()).filter(Boolean);
    }
  }

  if (result.reasons.length === 0 && result.hasEvaluation) {
    result.reasons.push("迁移时从历史记录推断");
  }

  if (result.hasEvaluation) {
    result.note = "规则判定（迁移）：" + (result.nextStatus || "未知") +
      (result.reasons.length > 0 ? "（" + result.reasons.join("；") + "）" : "");
  }

  return result.hasEvaluation ? result : null;
}

function inferRuleInfoFromObservation(obs, item, index) {
  const result = {
    reasons: [],
    nextStatus: null,
    note: "",
  ...
  const isAbnormal = obs.abnormal === true;
  const hasAbnormalKeyword =
    (obs.abnormalNote && (obs.abnormalNote.includes("霉") || obs.abnormalNote.includes("臭") || obs.abnormalNote.includes("结块"))) ||
    (obs.smell && (obs.smell.includes("霉") || obs.smell.includes("臭"))) ||
    (obs.fiber && obs.fiber.includes("结块"));

  if (hasAbnormalKeyword) {
    result.nextStatus = "异常观察";
    if (obs.abnormalNote?.includes("霉") || obs.smell?.includes("霉")) result.reasons.push("关键词异常：霉");
    if (obs.abnormalNote?.includes("臭") || obs.smell?.includes("臭")) result.reasons.push("关键词异常：臭");
    if (obs.abnormalNote?.includes("结块") || obs.fiber?.includes("结块")) result.reasons.push("关键词异常：结块");
  }

  if (obs.temperature) {
    const temp = Number(obs.temperature);
    if (!isNaN(temp)) {
      if (temp < 15 || temp > 35) {
        result.reasons.push("温度超限：" + temp + "℃");
        if (!result.nextStatus) result.nextStatus = "异常观察";
      }
    }
  }

  if (isAbnormal && !result.nextStatus) {
    result.nextStatus = "异常观察";
  }

  if (index === (item.observations || []).length - 1) {
    if (item.status === "可抄纸") {
      if (!result.nextStatus) result.nextStatus = "可抄纸";
      if (item.days >= (item.expectedDays || 7)) {
        result.reasons.push("天数达标：" + item.days + "天");
      }
    } else if (item.status === "发酵中" && !result.nextStatus) {
      result.nextStatus = "发酵中";
    } else if (item.status === "异常观察" && !result.nextStatus) {
      result.nextStatus = "异常观察";
    }
  }
  ...
  if (result.reasons.length === 0) {
    result.reasons.push("迁移时从观察记录推断");
  }

  result.note = "规则判定（迁移）：" + result.nextStatus + "（" + result.reasons.join("；") + "）";
...
export function migrateItemToEvents(db, item) {
  ...
  for (let i = 0; i < observations.length; i++) {
    const obs = observations[i];
    const obsTime = new Date(obs.at).getTime();
    ...
    for (const log of logs) {
      ...
      if (Math.abs(logTime - obsTime) < 5000 && log.step === "观察") {
        matchingLog = log;
        break;
      }
    }
    ...
    if (matchingLog) {
      ruleInfo = extractRuleInfoFromLog(matchingLog.note);
    }

    if (!ruleInfo) {
      ruleInfo = inferRuleInfoFromObservation(obs, item, i);
    }

    const isAbnormal = ruleInfo.nextStatus === "异常观察" || obs.abnormal === true;
    const ruleEvent = createEvent(
      ...
      batchCode,
      {
        ruleId: "migrated",
        ruleName: "迁移推断规则",
        nextStatus: ruleInfo.nextStatus || item.status,
        newDays: item.days || 0,
        reasons: ruleInfo.reasons,
        keywordMatched: ruleInfo.reasons.some(r => r.includes("关键词")),
        triggered: {
          abnormalChecked: isAbnormal,
          keywordAbnormal: ruleInfo.reasons.some(r => r.includes("关键词")),
          temperatureAbnormal: ruleInfo.reasons.some(r => r.includes("温度")),
        },
        note: ruleInfo.note,
        originalIndex: i,
      },
      {
        ...
        abnormal: isAbnormal,
        _migrated: true,
        _migratedFrom: "observations[" + i + "].rule_evaluation",
      }
    );
  ...
  events.sort((a, b) => {
    ...
    const aIdx = a.data?.originalIndex ?? 999;
    const bIdx = b.data?.originalIndex ?? 999;
    return aIdx - bIdx;
  });
...
export function verifyMigration(db) {
  ...
  for (const item of items) {
    ...
    itemResult.checks.eventCount = events.length;
    if (events.length === 0) {
    ...
    const hasCreateEvent = events.some(e => e.type === EVENT_TYPES.BATCH_CREATED);
    itemResult.checks.hasCreateEvent = hasCreateEvent;
    if (!hasCreateEvent) {
      itemResult.issues.push("missing_create_event");
      result.warnings.push("批次 " + batchKey + " 缺少创建事件");
    }

    const expectedObsCount = (item.observations || []).length;
    const obsEvents = events.filter(e =>
      e.type === EVENT_TYPES.OBSERVATION_RECORDED ||
      e.type === EVENT_TYPES.INSPECTION_RECORDED
    );
    itemResult.checks.expectedObservations = expectedObsCount;
    itemResult.checks.actualObservations = obsEvents.length;

    const expectedLogCount = (item.logs || []).length;
    const migratedEvents = events.filter((e) => e._migrated);
    itemResult.checks.expectedLogs = expectedLogCount;
    itemResult.checks.migratedEvents = migratedEvents.length;

    const minExpectedEvents = 1 + expectedObsCount + Math.max(0, expectedLogCount - 1);
    if (events.length < minExpectedEvents) {
      itemResult.issues.push(
        "event_count_insufficient: expected >= " + minExpectedEvents + ", got " + events.length
      );
      result.warnings.push(
        "批次 " + batchKey + " 事件数不足: 期望 >= " + minExpectedEvents + ", 实际 " + events.length
      );
    }

    const rebuilt = rebuildBatchState(db, batchKey);
    if (rebuilt) {
      itemResult.checks.rebuiltStatus = rebuilt.status;
      itemResult.checks.originalStatus = item.status;
      ...
      itemResult.checks.rebuiltDays = rebuilt.days;
      itemResult.checks.originalDays = item.days || 0;
      if (item.days !== undefined && item.days !== null && rebuilt.days !== item.days) {
        itemResult.issues.push(
          "days_mismatch: expected " + item.days + ", got " + rebuilt.days
        );
        result.warnings.push(
          "批次 " +
            batchKey +
      ...
      itemResult.checks.rebuiltObsCount = rebuilt.totalObservations;
      itemResult.checks.originalObsCount = itemObsCount;
      ...
      if (itemObsCount > 0 && rebuilt.latestObservation) {
        ...
        itemResult.checks.rebuiltLatestTemp = rebuilt.latestObservation.temperature;
        itemResult.checks.originalLatestTemp = lastOriginal.temperature;
        if (lastOriginal.temperature && rebuilt.latestObservation.temperature !== lastOriginal.temperature) {
          itemResult.issues.push(
            "latest_observation_temp_mismatch"
          );
          result.warnings.push(
            "批次 " + batchKey + " 最近观察温度不匹配"
          );
        }
      }

      const rebuiltLogCount = (rebuilt.logs || []).length;
      const originalLogCount = (item.logs || []).length;
      itemResult.checks.rebuiltLogCount = rebuiltLogCount;
      itemResult.checks.originalLogCount = originalLogCount;
      if (rebuiltLogCount < originalLogCount) {
        itemResult.issues.push(
          "log_count_mismatch: expected >= " + originalLogCount + ", got " + rebuiltLogCount
        );
        result.warnings.push(
          "批次 " +
            batchKey +
      ...
      const rebuiltAbnormalCount = rebuilt.abnormalCount || 0;
      const originalAbnormalCount =
        ((item.logs || []).filter(l => l.abnormal).length) +
        ((item.observations || []).filter(o => o.abnormal).length);
      itemResult.checks.rebuiltAbnormalCount = rebuiltAbnormalCount;
      itemResult.checks.originalAbnormalCount = originalAbnormalCount;
    }
