...
const fieldLabelsForCorrection = {
  code: "批次编号",
  temperature: "温度",
  smell: "气味",
  fiber: "纤维松散度",
...
export function autoCorrectFields(row) {
  const corrections = [];
  const corrected = { ...row };

  if (corrected.code !== undefined) {
    const originalCode = corrected.code;
    const trimmedCode = originalCode.trim();
    if (trimmedCode !== originalCode) {
      corrected.code = trimmedCode;
      corrections.push({
        field: "code",
        label: fieldLabelsForCorrection.code,
        original: originalCode,
        corrected: trimmedCode,
        reason: "去除前后空白字符",
      });
    }
    const upperCode = corrected.code.toUpperCase();
    if (upperCode !== corrected.code && /^[a-z]{2}-\d+$/i.test(corrected.code)) {
      corrections.push({
        field: "code",
        label: fieldLabelsForCorrection.code,
      ...
      corrected.code = upperCode;
    }
  }

  if (corrected.temperature !== undefined && corrected.temperature !== "") {
    const originalTemp = String(corrected.temperature);
    let cleanedTemp = originalTemp.trim();
    cleanedTemp = cleanedTemp.replace(/℃/g, "").replace(/°C/g, "").replace(/°c/g, "");
    cleanedTemp = cleanedTemp.replace(/摄氏度/g, "");
    cleanedTemp = cleanedTemp.trim();
    if (cleanedTemp !== originalTemp && !isNaN(Number(cleanedTemp))) {
      corrections.push({
        field: "temperature",
        label: fieldLabelsForCorrection.temperature,
        original: originalTemp,
        corrected: cleanedTemp,
        reason: "去除温度单位符号，保留数值",
      });
      corrected.temperature = cleanedTemp;
    }
  }

  if (corrected.changedWater !== undefined) {
    const originalWater = String(corrected.changedWater).trim();
    const waterYesKeywords = ["已换", "换过", "换了", "换水", "是", "有", "true", "TRUE", "True", "√", "✓"];
    const waterNoKeywords = ["未换", "没换", "没有", "否", "无", "false", "FALSE", "False", "×", "✗"];
    if (waterYesKeywords.includes(originalWater) && originalWater !== "是") {
      corrections.push({
        field: "changedWater",
        label: fieldLabelsForCorrection.changedWater,
        original: originalWater,
        corrected: "是",
        reason: '换水字段统一为"是"',
      });
      corrected.changedWater = "是";
    } else if (waterNoKeywords.includes(originalWater) && originalWater !== "否") {
      corrections.push({
        field: "changedWater",
        label: fieldLabelsForCorrection.changedWater,
        original: originalWater,
      ...
      corrected.changedWater = "否";
    }
  }

  if (corrected.abnormal !== undefined) {
    const originalAbnormal = String(corrected.abnormal).trim();
    const abnormalYesKeywords = ["有", "是", "异常", "true", "TRUE", "True", "√", "✓"];
    const abnormalNoKeywords = ["无", "否", "正常", "false", "FALSE", "False", "×", "✗"];
    if (abnormalYesKeywords.includes(originalAbnormal) && originalAbnormal !== "是") {
      corrections.push({
        field: "abnormal",
        label: fieldLabelsForCorrection.abnormal,
        original: originalAbnormal,
        corrected: "是",
        reason: '异常字段统一为"是"',
      });
      corrected.abnormal = "是";
    } else if (abnormalNoKeywords.includes(originalAbnormal) && originalAbnormal !== "否") {
      corrections.push({
        field: "abnormal",
        label: fieldLabelsForCorrection.abnormal,
        original: originalAbnormal,
      ...
      corrected.abnormal = "否";
    }
  }

  if (corrected.smell !== undefined) {
    const originalSmell = String(corrected.smell).trim();
    if (originalSmell !== String(corrected.smell)) {
      corrections.push({
        field: "smell",
        label: fieldLabelsForCorrection.smell,
        original: corrected.smell,
        corrected: originalSmell,
        reason: "去除前后空白字符",
      });
      corrected.smell = originalSmell;
    }
  }

  if (corrected.fiber !== undefined) {
    const originalFiber = String(corrected.fiber).trim();
    if (originalFiber !== String(corrected.fiber)) {
      corrections.push({
        field: "fiber",
        label: fieldLabelsForCorrection.fiber,
        original: corrected.fiber,
        corrected: originalFiber,
        reason: "去除前后空白字符",
      });
      corrected.fiber = originalFiber;
    }
  }
...
export function parseObservationText(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (hasData) {
      ...
      const { corrected, corrections } = autoCorrectFields(row);
      Object.assign(row, corrected);
      row._corrections = corrections;
...
export function previewBatchImport(db, parsed) {
  ...
  for (const row of parsed.rows) {
    ...
    const corrections = row._corrections || [];
  ...
  return {
    ...
    correctedCount: parsed.correctedRowCount || 0,
    allCorrections: parsed.allCorrections || [],
    fieldIssues,
    matched,
    unmatched,
    abnormalWarnings,
...
export function evaluateFermentationStatus(db, item, observation, customRule = null) {
  const rule = customRule || findRuleForSource(db, item.source);
...
export function tryEvaluateWithCustomRule(db, itemIdOrCode, observation, customRule = null) {
  const item = db.items.find((x) => x.id === itemIdOrCode || x.code === itemIdOrCode);
  if (!item) {
  ...
  const ruleToUse = customRule ? normalizeCustomRule(customRule, item.source) : null;
  const evaluation = evaluateFermentationStatus(db, itemSnapshot, observation, ruleToUse);

  return {
    success: true,
    item: itemSnapshot,
    observation,
    customRuleUsed: !!customRule,
    ruleName: evaluation.rule.name,
    ruleSource: evaluation.rule.source || "*",
    currentDays: itemSnapshot.days,
    newDays: evaluation.newDays,
    currentStatus: itemSnapshot.status,
    nextStatus: evaluation.nextStatus,
    statusChanged: itemSnapshot.status !== evaluation.nextStatus,
    daysChanged: itemSnapshot.days !== evaluation.newDays,
    isAbnormal: evaluation.isAbnormal,
    willBeReady: evaluation.willBeReady,
    reasons: evaluation.reasons,
    keywordMatched: evaluation.keywordMatched,
    temperatureCheck: evaluation.temperatureCheck,
    triggered: evaluation.triggered,
  };
}

function normalizeCustomRule(customRule, fallbackSource) {
  const defaultKeywords = ["霉", "臭", "腐", "酸败", "异味", "发黑", "结块", "霉斑", "发霉", "腐败"];
  const defaultAutoStatus = {
    onAbnormalKeyword: "异常观察",
    onTemperatureOutOfRange: "异常观察",
    onDaysReachedMin: "可抄纸",
    onDaysExceedMax: "异常观察",
  ...
  return {
    id: customRule.id || "custom-rule-preview",
    name: customRule.name || "自定义规则（试算中）",
    source: customRule.source || fallbackSource || "*",
    isDefault: (customRule.source || fallbackSource) === "*",
    minDays: Number(customRule.minDays) || 7,
    maxDays: Number(customRule.maxDays) || 30,
    temperatureMin: Number(customRule.temperatureMin) || 15,
    temperatureMax: Number(customRule.temperatureMax) || 35,
    abnormalKeywords: Array.isArray(customRule.abnormalKeywords)
      ? customRule.abnormalKeywords.filter((k) => String(k).trim() !== "")
      : defaultKeywords,
    autoStatusRules: {
      ...defaultAutoStatus,
      ...(customRule.autoStatusRules || {}),
    },
  };
}
...
export function getBatchHandoverDetails(db, batchIdOrCode) {
  const batch = (db.items || []).find((i) =>
    (i.id && i.id === batchIdOrCode) ||
    (i.code && i.code === batchIdOrCode)
  );
  if (!batch) return null;

  const batchKey = batch.id || batch.code;
  const allHandovers = getHandoversByBatch(db, batchKey);

  const pendingAbnormalitiesList = allHandovers
    .filter((h) => {
      const pa = String(h.pendingAbnormalities || "").trim();
      return pa && pa !== "无" && pa !== "暂无" && pa !== "没有";
    })
    .map((h) => ({
      handoverId: h.id,
      createdAt: h.createdAt,
      handedOverBy: h.handedOverBy,
      receivedBy: h.receivedBy,
      content: h.pendingAbnormalities,
    }));

  const latestWaterChange = allHandovers.find((h) =>
    String(h.nextWaterChangeReminder || "").trim() !== ""
  );

  const latestAbnormalObs = (batch.observations || [])
    .filter((o) => o.abnormal)
    .slice(-3)
    .map((o) => ({
      at: o.at,
  ...
  return {
    batch: {
      ...
      status: batch.status,
      days: batch.days || 0,
      expectedDays: batch.expectedDays || 0,
      startDate: batch.startDate || "",
    },
    handovers: allHandovers.map((h) => ({
      id: h.id,
      handedOverBy: h.handedOverBy,
      receivedBy: h.receivedBy,
      keyObservations: h.keyObservations,
    ...
    pendingAbnormalitiesList,
    latestWaterChangeReminder: latestWaterChange ? latestWaterChange.nextWaterChangeReminder : null,
    latestAbnormalObservations: latestAbnormalObs,
    handoverCount: allHandovers.length,
    lastHandoverAt: allHandovers.length > 0 ? allHandovers[0].createdAt : null,
  };
}
