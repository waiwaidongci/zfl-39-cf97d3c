...
export function analyzeBatchForComparison(db, item) {
  ...
  if (item.status === "可抄纸") {
    ...
    const start = parseDateExp(item.startDate) || parseDateExp(logs[0]?.at);
    if (start) {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const diff = Math.floor((today - start) / (1000 * 60 * 60 * 24));
      daysToReady = Math.max(0, diff);
    }
  }

  let waterChangeCount = observations.filter((o) =>
  ...
  let lastObservations = observations.slice(-3).map((o) => ({
    at: o.at,
  ...
  return {
    ...
    daysToReady,
    waterChangeCount,
    waterChangeFrequency: daysToReady && daysToReady > 0
      ? (waterChangeCount / daysToReady).toFixed(2)
      : "0",
    lastObservations,
