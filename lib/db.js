...
export async function loadDbWithMigration() {
  ...
  if (migrated1 || migrated2 || migrated3) {
    await saveDb(db);
  }
  return db;
}
...
export function validateHandover(input) {
  const errors = [];
  if (!input || typeof input !== "object") {
    return { valid: false, errors: ["交接记录数据格式错误"] };
  }
  ...
  const batchCodes = Array.isArray(input.batchCodes) ? input.batchCodes : [];
  if (batchIds.length === 0 && batchCodes.length === 0) {
...
export function createHandover(db, input) {
  const validation = validateHandover(input);
  if (!validation.valid) {
    return { success: false, errors: validation.errors };
  }
  ...
  for (const bid of inputBatchIds) {
    const item = db.items.find((i) => i.id === bid || i.code === bid);
    if (item) {
      const itemId = item.id || item.code;
      if (itemId && !batchIds.includes(itemId)) batchIds.push(itemId);
      if (item.code && !batchCodes.includes(item.code)) batchCodes.push(item.code);
    }
  }
  for (const code of inputBatchCodes) {
  ...
  for (const bid of handover.batchIds) {
    ...
    if (item) {
      ...
      item.logs.push({
        at: now,
        step: "交接",
        note: "交接给" + handover.receivedBy + "（交出人：" + handover.handedOverBy + "）",
      });
    }
  }
