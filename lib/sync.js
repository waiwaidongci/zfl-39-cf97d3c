...
export function detectConflicts(db, importPkg) {
  ...
  for (const remoteItem of remoteItems) {
    ...
    if (localItem.owner !== remoteItem.owner) {
      conflicts.push({
        id: "CFL-" + Date.now() + "-" + Math.random().toString(36).slice(2, 6),
    ...
    if (localItem.vat !== remoteItem.vat) {
      conflicts.push({
        id: "CFL-" + Date.now() + "-" + Math.random().toString(36).slice(2, 6),
...
export function applyMerge(db, importPkg, resolvedConflicts) {
  ...
  for (const remoteItem of remoteItems) {
    ...
    if (statusConflict?.resolved) {
      ...
      if (getLatestUpdateTime(remoteItem) > getLatestUpdateTime(localItem)) {
        updatedItem.status = remoteItem.status || updatedItem.status;
      }
    }
